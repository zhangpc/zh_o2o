...
def gettoken():
    print("登录用户")
    url_token = 'https://beta.console.xqshijie.com/access_token.json?username=keesh.zhang&password=Cc'
    try:
        content = requests.get(str(url_token),verify = False)
...
class RequestThread(threading.Thread):
    ...
    def getvalidate(self):
        global CONSTANT
        url_validate = "https://beta.console.xqshijie.com/customer/validate/"+str(self.customer_phone)+".json"
        try:
...
if __name__ == '__main__':
    ...
    xqsj_test = sqlconnect.get_xqsj_bate_sql() #
    fangfull_test = sqlconnect.get_fangfull_beta_sql()  #房否 test sql
    print("初始化完成")
    ...
    #     sem.acquire()
    #     t = RequestThread(req_session,bese_str,cellphone[i],'200',getxls,i,len(cellphone),'房否后台_老用户(beta)',customerid[i]).start()
    ...
    # for i in range(len(cellphone)):
    #     sem.acquire()
    #     t = RequestThread(req_session,bese_str,cellphone[i],'602',getxls,i,len(cellphone),'房否后台_已认购or已取消(beta)',customerid[i]).start()
    ...
    # for i in range(len(cellphone)):
    #     sem.acquire()
    #     t = RequestThread(req_session,bese_str,cellphone[i],'200',getxls,i,len(cellphone),'房否后台_新奇世界_老客户(beta)',customerid[i])
    #     t.start()
    #
    ...
    # for i in range(len(cellphone)):
    #     sem.acquire()
    #     t = RequestThread(req_session,bese_str,cellphone[i],'602',getxls,i,len(cellphone),'房否后台_新奇世界_已认购or已取消(beta)',customerid[i]).start()
...
#------------------------
    ...
    for i in range(len(cellphone)):
        sem.acquire()
        t = RequestThread(req_session,bese_str,str(cellphone[i]).replace(' ',''),'602',getxls,i,len(cellphone),'房否后台_没有订单记录的用户（beta）',customerid[i]).start()
...
#     for i in range(len(cellphone)):
#         sem.acquire()
#         t = RequestThread(req_session,bese_str,str(cellphone[i]).replace(' ',''),'602',getxls,i,len(cellphone),'新奇世界_非老用户(beta)',customerid[i]).start()
...
## 新奇世界老用户
    ...
    # customerid = ['108953']
    # cellphone = ['13600000103']
    # getxls = openxls()
    # maxThread = 8
    # sem=threading.BoundedSemaphore(maxThread-1)
    # for i in range(len(cellphone)):
    #     sem.acquire()
    #     t = RequestThread(req_session,bese_str,cellphone[i],'602',getxls,i,len(cellphone),'beta全民经纪人_13600000102_',customerid[i]).start()
